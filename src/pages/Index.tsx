...
const Index = () => {
  ...
  return (
    <SiteLayout>
      {/* ══════════════════════════════════════════════════════════════════
          HERO — видео оставляем как есть, поверх: floating cards + text
      ══════════════════════════════════════════════════════════════════ */}
      <div className="relative overflow-hidden" style={{ minHeight: 780 }}>
        ...
        <div className="absolute inset-0">
          ...
          {/* Radial overlay для читаемости текста */}
          <div
            className="absolute inset-0"
            style={{
              background: "radial-gradient(ellipse at 50% 60%, rgba(10,10,10,0.4), rgba(10,10,10,0.85) 70%)",
            }}
          />
        </div>
        ...
        <div className="relative max-w-[1320px] mx-auto px-8 pt-32 pb-24 text-center">
          ...
          <div
            ...
            style={{
              background: "rgba(180,120,253,0.12)",
          ...
          <h1
            ...
            style={{ fontSize: "clamp(64px, 9vw, 128px)", fontWeight: 400 }}
          >
            Все AI-модели
            <br />
            в <em className="text-accent not-italic">одном</em> чате
          </h1>

          {/* Subtitle */}
          <p
            className="text-[18px] max-w-xl mx-auto mb-10"
          ...
          <div className="flex items-center justify-center gap-3 mb-6">
            <Link
              to="/signup"
              className="px-6 py-3.5 rounded-xl text-[14px] flex items-center gap-2 transition-opacity hover:opacity-90"
            ...
            <Link
              to="/models"
              className="px-6 py-3.5 rounded-xl text-[14px] transition-colors hover:bg-white/10"
              style={{
      ...
      {/* ══════════════════════════════════════════════════════════════════
          MARQUEE — бесконечная лента моделей
      ══════════════════════════════════════════════════════════════════ */}
      <div
      ...
      >
        <div className="text-center text-[10px] tracking-[0.4em] uppercase mb-6" style={{ color: "rgba(250,250,250,0.42)" }}>
          Доступные модели
        </div>
        {/* Fade mask on sides */}
        <div
      ...
      <div className="max-w-[1320px] mx-auto px-8 py-24">
        <div className="text-center mb-16">
          ...
          <h2
            ...
            style={{ fontSize: "clamp(42px, 5vw, 72px)", fontWeight: 400 }}
          >
          ...
          {[
            {
              seed: "f1",
              title: "Одна цена",
              desc: "Платите поинтами. 1 пт = 1 ₽. Midjourney — 12 пт, Sora — 80 пт, Flux — 8 пт.",
            },
            {
              seed: "f2",
              title: "Без VPN",
              desc: "Российские реквизиты, СБП, карты. Работает из любой точки.",
            },
            {
              seed: "f3",
              title: "История и права",
              desc: "Всё в одной ленте. Коммерческие права на все генерации.",
            },
          ].map((f) => (
            <div
              key={f.title}
      ...
      {/* ══════════════════════════════════════════════════════════════════
          GALLERY — свежие работы сообщества
      ══════════════════════════════════════════════════════════════════ */}
      ...
      {/* ══════════════════════════════════════════════════════════════════
          CTA — «Генерируйте сейчас»
      ══════════════════════════════════════════════════════════════════ */}
      <div className="max-w-[1320px] mx-auto px-8 pb-24">
        <div
          className="rounded-[24px] p-14 text-center relative overflow-hidden"
          style={{
            background: "linear-gradient(135deg, #1a0a2e 0%, #2d0a4e 50%, #6b2a8a 100%)",
            border: "1px solid rgba(180,120,253,0.3)",
          }}
        >
          {/* Radial glows */}
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(circle at 20% 80%, rgba(180,120,253,0.5), transparent 40%), radial-gradient(circle at 80% 20%, rgba(106,223,255,0.3), transparent 40%)",
            }}
          />
          <div className="relative">
            ...
            <Link
              to="/signup"
              className="inline-block px-8 py-4 rounded-xl text-[15px] transition-opacity hover:opacity-90"
              style={{ background: "hsl(var(--foreground))", color: "hsl(var(--background))", fontWeight: 600 }}
            >
              Начать бесплатно →
            </Link>
      ...
      {/* keyframes for marquee (if not already in index.css) */}
      <style>{`
        @keyframes marquee {
